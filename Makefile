...
bintray: rpm_fonts deb_fonts
	$(foreach FONT,$(FONTLIST), \
		cd ${PRJDIR} && curl -T ${CURRENTDIR}/target/RPM_FONTS/${FONT}/RPMS/noarch/php-tecnickcom-${PROJECT}-data-${FONT}-${VERSION}-${RELEASE}.noarch.rpm -u${APIUSER}:${APIKEY} -H "X-Bintray-Package:${PROJECT}-data-${FONT}" -H "X-Bintray-Version:${VERSION}" -H "X-Bintray-Publish:1" -H "X-Bintray-Override:1" https://api.bintray.com/content/tecnickcom/rpm/php-tecnickcom-${PROJECT}-data-${FONT}-${VERSION}-${RELEASE}.noarch.rpm && \
